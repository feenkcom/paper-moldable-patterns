...
\wc{“Moldable” is a type of development, itself a process that requires decision-
making by someone, who?
A system made “explainable” provides better support for decision-making, but
how? And how much?
Jason Fried writes, "It's all a judgment call. Every human decision is a gut
decision. The decision itself is the messy integration of many disparate pieces of
information, experiences, instincts, stories, and unknowns.” 
...
\wc{Tools make software explorable by navigating a model, but is the model reliable?
Sufficiently complete?
The model will be in terms of the problem domain, but does the domain have
ubiquitous terminology?
The model will be live, meaning it will acquire experience like any living thing?
The tools will be cheap, but in what way? Inexpensive? Prone to breakage? Of
no lasting value?}
...
\wc{The authors base this publication on experience, direct personal experience in
each case? Reports from others?
Several repeating solutions have been identified and described as patterns using
words and pictures.
The patterns depend on each other in order to provide solutions, presumably
leading to explanations.
The patterns explain the development process, but not the system that is
explained by the process leading to decisions.
I now leap to the conclusion and continue careful reading. I can’t be critical of
omission now since these sentences complete the document, rather than
introduce it.}
\end{abstract}
...
\eog{I think the biggest pieces of feedback I've got are centered around as a person who comes from industry like web development, there's terms in here that I would not consider in the vernacular of a like normal industry developer.
...
\eog{Scaling to arbitrary systems.
I'm not sure what that means.}

\rb{Okay, what we think is missing from the paper is the problem statement of Moldable Development. This is excellent:
...
\rb{The Human Nature that Ward would like to see in problem statements could easily explore short-term vs long-term decisions. For example, molding is for the short term, but the information it exposes could be either, maybe even both, short for management, long for full time employees.
...
These custom tools consist of small extensions to the \emph{moldable} tools of the integrated development environment (IDE) \tg{could we introduce here the moldable development environment as a concept?}, such as the object inspector, the code editor, the debugger and the notebook.

\eog{For example, an object inspector also not a thing in the vernacular from where I come from.}

\eog{So I think one of the first things that popped out was this idea of decision making is a thing.
If you talk about it some more than like I get it.
But the idea of decision making as a thing wasn't something for me until I started to work at feenk.}
...
The figure shows three connected custom inspector views of a running game in Glamorous Toolkit(\GT)~\cite{feenk23a}, a moldable development environment \tg{here we talk about MoDE, but later we refer to IDE. I think it would be nice to introduce MDE earlier and talk about that.}.
In the leftmost pane we see the game GUI as the \st{Board} view.
We can also interact programmatically with the game, evaluating ``\st{self autoPlay: 1000}'' in a contextual playground (a kind of REPL) below the view.\tk{REPL?}
...
Moldable development can be understood in terms of a collection of mutually supporting \del{design and process} \tg{the term design introduces a new category that we do not talk about later on - it's somehow overlapping tooling and modelling. I would talk here only about patterns and only and only later introduce the difference categories} patterns, in other words, a \emph{pattern language}, summarized in \autoref{fig:map}.
...
The patterns that are concerned with \emph{Tooling} (tagged with ``T''), with \emph{Modeling} (M), or with the development \emph{Process} (P).
\kh{Page 1: "The patterns that are concerned with Tooling (tagged with
...
What a domain model really was like a model to me was the M in MVC.}
...
\kh{Page 2: "Generic software tools are fine for answering generic
...
We leverage a \pattern{customView} of the \st{MovieCollection} and of the \st{Movie} to aid our exploration.
\eog{I don't know how you feel about language notes like this, like we leverage.
...
\eog{I really like this part talking about the inspection of a live web page.
...
\kh{I find the "modeling patterns" most interesting, because they show how
...
different from the usual focus on the evolution of a code base. }
...
\eog{Moldable object context, you have got your basic infrastructure in place.
...
\eog{When you write code in a classical IDE, I only know one type of IDE.
...
\eog{And then this just, this is like huge, this is awesome.
...
\eog{In most cases, you can't immediately start building the custom tools.
...
\eog{The programming in the live REPL is huge and a big deal different from the usual like it is a batch process approach.
...
\eog{Learning to explicitly ask questions is like, again that asking a fish about water.
...
Unlike normal test methods, examples can be \emph{composed}.
...
\eog{Unlike normal test methods, examples can be composed.
...
\kh{My personal highlight is the "Moldable Collection Wrapper". I have been
...
\eog{So how do you develop custom tools for existing data?
...
\eog{Would love to see the goal or motivation earlier.
...
\eog{A problem that is not understandable.
I have trouble understanding with that is that could look like.}
...
\wc{Live programming is a paradigm shift.
...
\wc{GT embodies these patterns.
The authors believe the patterns have utility not tied to GT, but are they sufficient
to match GT when applied?
To be useful the patterns require some aspects of live programming, which
aspects?
This last sentence is sufficient reason for the makers of other environments to
pay attention to this work.The claim is qualified as only a belief of the authors.
Surely these patterns will prove valuable should the authors of GT undertake GT
2.0 and want to be sure not to lose something important. I am left wondering
what sort of project should be undertaken in my own environment and how that
project's result should be evaluated.}

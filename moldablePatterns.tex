...
%\wc{“Moldable” is a type of development, itself a process that requires decision- making by someone, who? A system made “explainable” provides better support for decision-making, but how? And how much? Jason Fried writes, "It's all a judgment call. Every human decision is a gut decision. The decision itself is the messy integration of many disparate pieces of information, experiences, instincts, stories, and unknowns.” \url{https://world.hey.com/jason/it-s-all-a-judgment-call-6ab5d025}} \todo{Explain what we mean by decision making.}
...
%\wc{Tools make software explorable by navigating a model, but is the model reliable? Sufficiently complete? The model will be in terms of the problem domain, but does the domain have ubiquitous terminology? The model will be live, meaning it will acquire experience like any living thing? The tools will be cheap, but in what way? Inexpensive? Prone to breakage? Of no lasting value?}
...
%\wc{The authors base this publication on experience, direct personal experience in each case? Reports from others? Several repeating solutions have been identified and described as patterns using words and pictures. The patterns depend on each other in order to provide solutions, presumably leading to explanations. The patterns explain the development process, but not the system that is explained by the process leading to decisions. I now leap to the conclusion and continue careful reading. I can’t be critical of omission now since these sentences complete the document, rather than introduce it.}
\end{abstract}
...
%\eog{I think the biggest pieces of feedback I've got are centered around as a person who comes from industry like web development, there's terms in here that I would not consider in the vernacular of a like normal industry developer. And I think that it might get in the way of doing what you're trying to do.} \todo{Change terminology or elaborate where appropriate}
...
The usual \ins{ways to view} software systems are
\begin{inparaenum}[(i)]
...
%\eog{Views. I don't think that's a thing in vernacular, like the closest you'll find it is in the like V like where I would sit my HTML templates.} % fixed
%\eog{Scaling to arbitrary systems. I'm not sure what that means.} % rephrased

%\rb{Okay, what we think is missing from the paper is the problem statement of Moldable Development. This is excellent: \url{https://book.gtoolkit.com/moldable-development-ejn67l5cdqh83kegi9umt1mdd} “When we think of software development, we typically think about the active part. Of constructing. Of building new worlds that never existed. It's an empowering view. Yet, Developers spend most of their time figuring systems out. They do that because they want to learn enough about the system to make a decision. This is the single largest development expense we have. So, we should optimize software engineering for decision making.”} % added paragraph from GT book with citation Zelk79b

%\eog{So I think one of the first things that popped out was this idea of decision making is a thing. If you talk about it some more than like I get it. But the idea of decision making as a thing wasn't something for me until I started to work at feenk.}

\ins{When we think of software development, we typically think about the active part.
Of constructing.
...
%\rb{The Human Nature that Ward would like to see in problem statements could easily explore short-term vs long-term decisions. For example, molding is for the short term, but the information it exposes could be either, maybe even both, short for management, long for full time employees. I probably need to explain the Human Nature thing in more detail. It has to do with how he thinks patterns should be written.}
...
\ins{Every part of a software system deals with a particular \emph{domain}, whether this be the \emph{business domain} of the overall application, or a more \emph{technical domain}  of the underlying application.
...
%\eog{Domain models. I got into talking about domain models, like I wanna say, 12 years in. And until then I just, like, never heard about domain driven development. What a domain model really was like a model to me was the M in MVC.}
%\tg{explainability extends to other parts that would not be considered domain model. For example, a static dependency graph shows how the system is composed based on a model, but I believe it would not be understood to fall under the domain model. Either we explain what we mean by domain model to also include such variations, or we choose another term.}

\ins{The Glamorous Toolkit (GT)~\cite{feenk23a} is an example of a \emph{moldable} \emph{development} \emph{environment} in which the development tools are continuously molded to make the software under development explainable.
The examples we use to illustrate moldable development in this paper are all drawn from GT.}

%\tg{could we introduce here the moldable development environment as a concept?}
%\eog{For example, an object inspector also not a thing in the vernacular from where I come from.}
...
%\kh{the examples are well chosen (but the images are a bit small, you need to zoom in to read the text)}
...
% Glamorous Toolkit(\GT)~\cite{feenk23a}, a moldable development environment \tg{here we talk about MoDE, but later we refer to IDE. I think it would be nice to introduce MDE earlier and talk about that.}. \todo{FIX THIS -- refer to GT earlier}
In the leftmost pane we see the game GUI as the \st{Board} view.
We can also interact programmatically with the game, evaluating ``\st{self autoPlay: 1000}'' in a contextual playground (a kind of \chg{REPL}{interactive shell}) below the view. %\tk{REPL?}
...
%\rb{Ward feels that the paper, the PDF, has a certain need to stand alone. Most of what he knows about GT comes from the conversations we had with Tudor. He was very impressed with how Tudor said you guys would send GT quotes back and forth via Slack, with little or no accompanying explanation. You also say "explanation" a lot. The summary/abstract starts and ends with that word. Oscar, at this point I had pointed out your Narrative Picture pattern to Ward and noted that it is not (yet) part of the paper.}
...
%\del{design and process} \tg{the term design introduces a new category that we do not talk about later on - it's somehow overlapping tooling and modelling. I would talk here only about patterns and only and only later introduce the difference categories} 
...
%\kh{Page 1: "The patterns that are concerned with Tooling (tagged with “T”), with Modeling (M), or with the development Process (P)." Parse error: not a proper sentence.}
...
Each custom tool \chg{requires the existence of a}{can be seen as an extension of an existing} \pattern{moldableTool}, which can be cheaply adapted by a simple customization.
%\eog{Each custom tool requires the existence of a moldable tool. By itself I don't understand what that means. And I'd like to see a little more maybe in terms of like, I don't know, explanation or motivation for what that is.}
...
%\eog{So a moldable object can be explored. I don't think that's a thing in vernacular like I'm saying this, but I'm coming from a place where like if you talk about it and you explain it, you're like, yes, I do do that. I just like, didn't have a name for it. And it's, I feel like a lot of this is also related to that idea of like, it's like talking to fish about water, like the idea of explainability views and this kind of stuff like software developers do this, but they just don't have awareness that they're doing it.}
...
Generic software tools are fine for answering generic questions, but they do not \chg{scale}{work} well when addressing domain-specific questions.
%\eog{Another thing about, like, generic software tools, I don't know what that is. Scale well, I don't know what that means.}
...
%\kh{Page 2: "Generic software tools are fine for answering generic questions, but they do not scale well when addressing domain-specific questions." The transition from generic to domain-specific is not "scaling" by my definition.}
...
%\eog{Page three, you have a moldable object and you're ready to start exploring. Where does this fit in with a normal job or workflow? I think like if you read the whole paper and then you chew on it and then you read it again, the stuff makes sense, but it's difficult to get there the first pass through and maybe that's OK, maybe it's not important?}
...
The playground will be bound to the context of the instance, so \st{self} and all slots (\ie \chg{fields}{instance variables}) can be accessed exactly as they would in a running method.
%\eog{Right fields, maybe you want to refer to instance variables, that's what or like whatever they're called in Python, for example, that may be a way to make this more accessible.}
...
We \chg{leverage}{use} a \pattern{customView} of the \st{MovieCollection} and of the \st{Movie} to aid our exploration.
%\eog{I don't know how you feel about language notes like this, like we leverage. You could also say we use}
...
%\eog{I really like this part talking about the inspection of a live web page. Like that is a thing that I can think of. Like a lot of people being intimately familiar with and it is a great metaphor or like link to those kind of patterns descriptions that you're referring to.}
...
As an example, consider the inspector view in \autoref{fig:customLePageAction} of a notebook page in Lepiter~\cite{Girb21a}, \ins{the live documentation system of GT.}
%\kh{The name "Lepiter" occurs 10 times, but is never properly introduced.}
...
%\kh{I find the "modeling patterns" most interesting, because they show how Moldable Development goes beyond the developer productivity issues that most IDEs focus on. They also provide a perspective on OO that is very different from the usual focus on the evolution of a code base. }
...
%\eog{Moldable object context, you have got your basic infrastructure in place. I don't know what that is and I would love a little pointer or more of an explanation.}
...
%\eog{When you write code in a classical IDE, I only know one type of IDE. And so when you talk about classical, I think this, this gets in your way, you like in the way of your goal more than it adds start coding by inspecting a moldable object.}
...
%\eog{And then this just, this is like huge, this is awesome. I'd love an article just on doing this. I wonder if there's a way that we can like push this up because this is this is the stuff, this is like what got me into talking to Tudor in the first place.}
...
%\eog{In most cases, you can't immediately start building the custom tools. So like here's like more stuff like I wish this went further 1st, 1st, further up. Right, and turning the exploration of an object into a custom tool. Like this is, this is really big.}

%\eog{The programming in the live REPL is huge and a big deal different from the usual like it is a batch process approach. Of making changes and then guessing and tracking. Like that's how a lot of work got done at Shopify. I would watch people, like, maybe they'd have their exploration done in a test, like at best, maybe in a console. But there would be a lot of like, does this work and then you'd have to, like, go to your editor and make some changes. Hope. The reloading was very unreliable and then, you'd have to like restart the whole system and doing that. It, it just took forever. It sucked. And so when I saw GT at work, like that was magic.}
...
% \eog{Learning to explicitly ask questions is like, again that asking a fish about water. I talked about that at the beginning. I also find that this could be like a paper unto itself like, hey, have you tried this?}
...
Unlike normal test methods, examples \chg{can be}{are designed to be} \emph{composed}.
...
%\eog{Unlike normal test methods, examples can be composed. I would say that examples are designed to be composed.}
...
%\kh{My personal highlight is the "Moldable Collection Wrapper". I have been fighting a few times with the tension between "I want to add views to this" and "I want to have standard collection protocols on this". Implementing an adaptor as a trait looks like an obvious solution - after reading the paper.}
...
%\eog{So how do you develop custom tools for existing data? And you're like, yeah, you wrap them and then you build this stuff up. I think what I would like to see is more of that. Like here's the cool thing that you get out of this, like the value the the upside earlier maybe that just doesn't fit the the pattern that you're filling in like this template. But later on, you're like, look at this cool thing, maybe that's the consequences.}
...
\end{figure}
%\tg{The picture shows also a gray hover highlighting. It should only show the selected one}
...
\subsubsection*{Context}
You are assessing a problem such as a bug, a performance problem, or a security issue, and you are facing a problem that is \chg{not understandable}{very difficult to understand}.

%\eog{A problem that is not understandable. I have trouble understanding what that is, and what that could look like.}
...
%\wc{Live programming is a paradigm shift. The shift is required to explore. The shift allows one to mold the model as explored. The molding is done in increments.}
...
\ins{The key to supporting moldable development is the pattern \pattern{moldableTool}.

...
The usual \chg{ways to view}{artifacts to evaluate} software systems are
\begin{inparaenum}[(i)]
...
This is achieved by making it \chg{cheap}{inexpensive} to create dozens, hundreds or even thousands of custom tools to answers questions about a software system \emph{as these questions arise}.
...
\del{The }Glamorous Toolkit (GT)~\cite{feenk23a} is an example of a \emph{moldable} \emph{development} \emph{environment} in which the development tools are continuously molded to make the software under development explainable.
...
Each custom tool can be seen as an extension of an existing \patref{MoldableTool}, which can be \chg{cheaply adapted by a simple customization}{inexpensively customized}.
...
The Moldable Coder~\cite{Syre17a} (\ie code editor) also can add custom views, \ins{highlighters,} actions and searches when editing the source of a class with annotated methods.
...
In the leftmost pane we see a notebook page with live code snippets, which function as contextual playgrounds within the context of the notebook page \tg{I am not sure we should call the global pages contextual. I somehow would keep that word only for the ones in a non-global context - like an inspector or debugger}.
...
At the bottom of the inspector we see \del{another} contextual playground, this one bound to the context of the Python object.
...
\tg{I think I would call this Contextual View to have it match the Contextual Playground. And I would also name the other two Contextual Search, Contextual Action. What do you think?}
...
\patsec{Problem}
\emph{How do you make it easy to find interesting information again?} \tg{I am not certain about the *again* part. Most often we want a view to be useful the first time we use it. The second time is a bonus.}
...
In a standard image from August 2024, there exist over 1800 classes with a total of over 3600 view methods, averaging under 12 lines of code \tg{is it worthwhile to say that these extensions affect 12K classes due to subclassing. What about adding the treemap with all classes with views and examples highlighted?}.
...
%: ----- Composed Narratives -------------------------
\pattern{Composed Narratives}{ComposedNarratives}
\tg{Composable Narratives?}
...
\patsec{Known Uses}
Lifeware\footnote{\url{https://www.lifeware.ch}} uses Moldable Data Wrappers extensively to wrap SQL data in the \chg{Insurance}{insurance} domain to be able to create custom views for these data.
...
\ins{Personal knowledge management systems like RoamResearch\footnote{\href{https://web.archive.org/web/20240820082136/https://docs.jupyter.org/en/latest/}{https://roamresearch.com}} are regularly used for diary including for software projects.}

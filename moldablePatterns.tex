...
\ws{JN: Is the business model separate from the display? (cf MVC) 
...
\c[{it could be a composite pattern}
...
and they often do not play nicely with existing tools or with each other.
...
\ws{JN: P. 7, line 119. Since “moldable” and “dynamic” are in italics. Are they forces?
...
\cp{such as?}
\item a moldable debugger to adapt the stepping behavior for domains such as event-driven applications, 
or parsing rules, 
...
You need a \patref{moldableTool} into which you can dynamically plug custom views.
...
You need a \patref{moldableTool} 
...
into which you can dynamically plug custom actions.
...
\patsec{Solution}
Start coding by inspecting a moldable object, that is, a live instance of the class you are coding, 
\cp{is any live instance of a class already a moldable object?}
\emph{not in a code editor}.
Then incrementally pose domain-specific questions, find the answers by exploring and interacting with the object, and then turn those answers 
...
Custom tools help make analyses faster, while generic, ``reusable'' tools 
...
can make it more difficult to find the answers you need.
...
\todo{Thanks to Petr Picha, Cesare Pautasso, and Daniel Pinho for their written comments, and the EuroPLoP workshop participants for their feedback.}

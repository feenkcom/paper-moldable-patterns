...
\copyrightyear{2024}
\acmYear{2024}
\setcopyright{acmlicensed}\acmConference[EuroPLoP 2024]{29th European Conference on Pattern Languages of Programs, People, and Practices}{July 3--7, 2024}{Irsee, Germany}
\acmBooktitle{29th European Conference on Pattern Languages of Programs, People, and Practices (EuroPLoP 2024), July 3--7, 2024, Irsee, Germany}
\acmDOI{10.1145/3698322.3698327}
\acmISBN{979-8-4007-1683-6/24/07}
%\copyrightyear{2024}
%\acmYear{2024}
...
\ifthenelse{\boolean{preprint}}{% FOR PREPRINT
\setlength{\itemizeMargin}{12pt} % for Forces
}{% FOR CAMERA-READY
\setlength{\itemizeMargin}{10pt} % for Forces
}
% ============================================================
\begin{document}
\ifthenelse{\boolean{preprint}}{% FOR PREPRINT
\title[Moldable Development Patterns --- preprint]{Moldable Development Patterns}%
\thanks{Preprint of paper workshopped at \href{https://www.europlop.net}{EuroPLoP} 2024, July 3--7, 2024, Kloster Irsee, Germany. To appear (ACM)}%
}{
...
Moldable development itself starts with a moldable object, a live instance of a domain entity that is explored and molded with custom tools that package the results of exploration tasks.
...
\begin{figure}[h]
  ...
  \caption{Defining a custom view in a just a few lines of code.}
  \label{fig:viewCode}
\end{figure}
...
\item When we code new behavior as methods, we must repeatedly change our context to incrementally develop the logic.
...
\patsec{Forces}
\begin{itemize}[leftmargin = 10pt, label = ---]
\item To obtain an interesting view of data, you are generally forced to activate a dedicated tool.
...
%\on{The point is that we do not want to have to always go to a tool to do something, but rather than views will enable the ability to get to interesting stuff just by navigating.}
...
From this custom view we can then dive directly into the \st{Move} object, which in turn has a custom view visualizing the change in state.
...
\begin{figure}[h]
  ...
  \caption{A treemap of all classes in GT grouped in packages, highlighting in blue those that define at least a Custom View.}
...
\patsec{Related patterns}
A custom view is closely related to a given \patref{MoldableObject}.
Interconnected objects with custom views can form a \patref{ComposedNarrative}.
...
\patsec{Examples}
In \autoref{fig:addressBookSearch} we see custom searches for people and addresses within an address book.
...
\begin{figure}[h]
  \includegraphics[width=\columnwidth]{addressBookSearch}
  \caption{Custom substring searches.}
  \label{fig:addressBookSearch}
\end{figure}
...
For example, \autoref{fig:viewCode} shows how a given Custom View is implemented in a few lines of code.
\autoref{fig:rawViewVsCustomView} shows how a raw view differs from a Custom View.
\autoref{fig:composedExample} illustrates how examples can be composed to create new examples.
...
\patsec{Known Uses}
Narrative art works,\footnote{\href{https://web.archive.org/web/20240916005104/https://en.wikipedia.org/wiki/Narrative\_art}{https://en.wikipedia.org/wiki/Narrative\_art}} from the Bronze Age through to modern graphic novels, tell stories by juxtaposing images.
...
\begin{figure}[h]
  ...
  \caption{Three interconnected moldable objects playing the role of a logger linked to a debugger.}
...
\item building a bridge to a foreign execution platform, such as for Python, AWS, or a database system, so that you can observe and interact with run-time entities in the target platform, or
...
However, assessment benefits from \emph{contextual tools}.
...
\patsec{Known Uses}
There are numerous examples of \emph{ad hoc} analyses implemented as Throwaway Tools in the GT book, for example the page on \emph{Optimizing the links in the book for first time readers} shows a script (\autoref{fig:optimizingLinks}) to generate a visualization of the pages in the book itself that are reachable by following links contrasted with standalone pages.

\patsec{Related patterns}
Use a \patref{ContextualPlayground} to prototype the tools.

\begin{figure}[ht]
  \includegraphics[width=\columnwidth]{optimizingLinks}
  \caption{A throwaway tool to visualize reachable and unreachable knowledge base pages.}
  \label{fig:optimizingLinks}
\end{figure}

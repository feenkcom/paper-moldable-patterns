...
In the leftmost pane we see the game GUI as the \ct{Board} view.
We can also interact programmatically with the game, evaluating ``\ct{self autoPlay: 1000}'' in a contextual playground (a kind of REPL).
In the second pane we can explore the moves of  the completed game, and in the third pane we can explore individual moves.
The \ct{Move} view visualizes the actual move performed in the context of the current game state at that point in time.
...
To start, you need a modified unit testing framework in which tests return the exercised fixture, namely, an example. In GT, you create an Example by defining a parameterless method that has a \ct{gtExample} pragma, and returns an object. (A similar framework for Java is JExample.)

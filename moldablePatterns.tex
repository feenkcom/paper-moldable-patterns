...
Each custom tool can be seen as an extension of an existing \patref{MoldableTool}, which can be cheaply adapted by a simple customization.
...
A \patref{ProjectDiary} is a notebook that serves as a starting point for development tasks.
...
A plugin architecture can open up an IDE to new tools, but plugins can be complex and expensive to implement, 
...
Make the development tools \emph{moldable} to the \emph{dynamic context} of the artifacts they are intended to work with~\cite{Chis17a}, by associating custom behavior to the artifacts themselves.
...
\patsec{Known Uses}
Unit testing tools such a JUnit~\cite{Beck98a} rely either on inheritance or annotations to automatically detect which methods in a package are tests, and adapt the IDE to enable testing tools, however the tool adaptation is fixed, and cannot be further customized without writing a new plugin.
...
A \patref{MoldableTool} reacts to a \patref{MoldableObject}, which triggers custom tools such as \patref{CustomView}, \patref{CustomSearch} or \patref{CustomAction}.
...
\patsec{Context}
You are using a \patref{MoldableTool}, and you are ready to start exploring.
...
\patsec{Solution}
\todo{Explain the relationship between exploring, coding and protoyping.}
Prototype new behavior in the contextual playground of the inspector on a \patref{MoldableObject}.
...
\todo{Add more explanation of what we are seeing!}
...
In the contextual playground (bottom of middle pane) we experiment with Python code to extract a particular movie.
We use a \patref{CustomView} of the \st{MovieCollection} and of the \st{Movie} to aid our exploration.
...
\todo{REPLs?
Web console.
GT playgrounds for inspector, coder and debugger.}

\patsec{Related patterns}
\todo{Prototyping Custom tools and extracting the code.
\patref{MoldableTool} and \patref{MoldableObject}.
\patref{CustomView}
\patref{CustomAction}
\patref{CustomSearch}
}
...
\patsec{Forces}
Navigating to the data you want to reach may entail a sequence of operations, either clicking in views, or evaluating Playground snippets,
%\cp{this is only implicitly hinted at in the previous pattern. Consider explaining it already in the playground pattern}
to reach the answer you seek.\\
...
Start with a \patref{SimpleView}.
...
\patsec{Related patterns}
\todo{...
\patref{MoldableTool}is a prerequisite. (Explain how)
\patref{SimpleView}.
}
...
\patsec{Known Uses}
\todo{
GT global spotter vs dedicated spotter for a knowledge base or a given inspector.
}

\patsec{Related patterns}
\todo{\patref{MoldableTool}.
\patref{CollectionWrapper}.
}
...
of creating an explainable system, either from scratch, or based on some existing software or data.

\todo{Refine context and problem.}

\patsec{Problem}
Where do you start coding an explainable system?

\patsec{Forces}
As a programmer, you want to quickly get feedback 
\cp{get feedback from whom? (maybe) understand the consequences/impact of the code you are writing.}
about the code you are writing.\\
...
\patsec{Solution}
Start coding by inspecting a live instance of the class you are coding, 
%\cp{is any live instance of a class already a moldable object?}
\emph{not in a code editor}.
Then incrementally pose domain-specific questions, find the answers by exploring and interacting with the object, and then turn the way you reach those answers 
%\cp{the way to obtain those answers (?)}
into custom tools, behaviors and tests.
...
\patsec{Solution}
Wrap examples as (instance) methods that optionally evaluate some tests (assertions), 
...
\todo{
\patref{MoldableObject}
NB: \patref{MoldableTool} is something else
}
...
\patsec{Known Uses}
\todo{
Need some GT anecdotes?
}

\patsec{Related patterns}
\todo{
\patref{MoldableObject} --- how?
}
...
\todo{Move the following sentences to other sections ... (Consequences etc}
...
\patsec{Related patterns}
\todo{
\patref{ContextualPlayground} to prototype the tools
}

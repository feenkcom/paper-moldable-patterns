...
In a nutshell, Moldable development is a way to make systems explainable by making the domain model \tg{the domain model is only a part of what is interesting inside a system. Could we use the term ``inside of'' instead?} of a software system visible and explorable through custom tools.
...
The patterns are mostly design patterns, showing how to tackle a design problem in moldable development, but many can also be interpreted as process patterns, that is, tasks and activities to perform during moldable development.

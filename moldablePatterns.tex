...
\subsubsection*{Context}
You are starting a new moldable development project for a customer with an existing code base.
...
\subsubsection*{Solution}
Rapidly develop a throwaway analysis tool that clearly demonstrates the advantages of turning a part of the target system into an explainable system.
...
Pick an aspect of their system that is causing problems, and apply moldable development to build a throwaway analysis tool that exposes the parts that are currently hard to understand.
Since the stakeholder has little time to spare to guide you, you may have to work in the dark at times.
Try to always have something to show when you come back with questions.
...
This means that you should feel ready to throw away whatever tool you develop and start again with the stakeholder on board.
...
\begin{figure}[h]
  \includegraphics[width=\columnwidth]{playerArolls6}
...
\subsubsection*{Problem}
How do you develop custom tools for 



When you explore data with a platform such as GT, the views you obtain reflect the low-level data structures used to represent the data, not the underlying domain concepts.
How can we add appropriate high-level views to the data?

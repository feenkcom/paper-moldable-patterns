...
By the same token an Inspector can be made moldable by recognizing that an object it is inspecting has one or more custom views defined as methods with a \st{<gtView>} annotation.

\on{add explanation}
\autoref{fig:viewCode}
...
\on{Other examples: Notebook, Debugger, Coder}
...
If needed, add a code snippet for any setup tasks (e.g., cloning repositories or loading databases).
Add code snippets to the notebook pages to serve as moldable objects to start coding from.
...
\on{Work on this one next.}
\subsubsection*{Context}
You need to start coding.

\subsubsection*{Problem}

\todo{Need a single statement.}

When you code a class, you are several steps away from seeing the consequences of your coding.
How can you close the gap between the code and the live objects?
...
\todo{How to get the live object?
From a playground. From a code snippet. From an example.
}
